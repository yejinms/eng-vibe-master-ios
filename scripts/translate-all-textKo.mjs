...
async function main() {
  ...
  for (const option of allOptions) {
    ...
    // textKo가 영어로 시작하는 경우
    if (/^[A-Za-z]/.test(option.textKo.trim()) && option.textKo === option.text) {
      needsTranslation++;
    }
  }
  
  console.log(`Options needing translation: ${needsTranslation}`);
  ...
  for (const option of allOptions) {
    ...
    if (option.correct && option.intent) {
      ...
      if (intentPhrase && intentPhrase !== newTextKo) {
