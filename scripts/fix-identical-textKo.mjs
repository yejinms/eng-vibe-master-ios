...
async function main() {
  ...
  const identicalOptions = allOptions.filter(opt => opt.text && opt.textKo && opt.text === opt.textKo);
  console.log(`Found ${identicalOptions.length} options with identical text and textKo`);
  
  if (identicalOptions.length === 0) {
    console.log('No identical text/textKo found.');
    return;
  }
  ...
  for (const option of identicalOptions) {
    ...
    // 2. intent에서 추출하지 못한 경우 번역
    if (newTextKo === option.text) {
    ...
    // 3. 반말/존댓말 조정
    if (newTextKo && newTextKo !== option.text) {
    ...
    if (newTextKo && newTextKo !== option.textKo) {
      ...
      console.log(`Line ${option.textKoLineIndex + 1}: "${option.text}" -> "${newTextKo}"`);
      
      if (updated % 10 === 0) {
        console.log(`Updated ${updated}/${identicalOptions.length}...`);
      }
    }
  }
